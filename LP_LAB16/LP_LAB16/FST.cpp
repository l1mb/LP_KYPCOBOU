...
void FST::Recognize(std::vector<std::pair<int, std::string>> lexems, std::ostream* logfile, std::ofstream* outputFile, LT::LexTable& lextable, IT::IdTable& idtable) {
#pragma region "Переменные"
	...
	FST fst_arr[] = { FST_MAIN , FST_PRINT , FST_RETURN , FST_DECLARE , FST_INTEGER , FST_STRING , FST_FUNCTION ,
		...
		FST_DIVISION , FST_COMMA, FST_SEMICOLON, FST_EQUALLY, FST_NUMBERS, FST_ID, FST_STRING_LITERAL, FST_LEFTSQUARE, FST_RIGHTSQUARE,
		FST_PERCENT };
	int size = sizeof(fst_arr) / sizeof(FST), it_id = -1;
	bool param = false, _main = false;
#pragma endregion

#pragma region "Разбор лексем"
	for (size_t i = 0; i < lexems.size(); i++) {
		if (i >= 1 && lexems[i - 1].first < lexems[i].first)
			str += IN_CODE_ENDL + _string(lexems[i].first) + ' ';
...
#pragma region "Определение IDDATATYPE"
				...
				if (fst_arr[j].symbol == LEX_LITERAL)
					IT::is_digit(lexems[i].second.c_str()) ? iddatatype = IT::IDDATATYPE::INT : iddatatype = IT::IDDATATYPE::STR;
...
#pragma region "Запись идентификаторов"
				if (fst_arr[j].symbol == LEX_ID) {
					if (it_id == TI_NULLIDX)
						if (lextable.size >= 2 && lextable.table[lextable.size - 2].lexema == LEX_DECLARE &&
							lextable.table[lextable.size - 1].lexema == LEX_DATATYPE) {
							IT::Add(idtable, { lextable.size, lexems[i].second.c_str(), current_prefix.c_str(), iddatatype, IT::IDTYPE::V });
						}
						else if (lextable.size >= 1 && lextable.table[lextable.size - 1].lexema == LEX_FUNCTION) {
							IT::Add(idtable, { lextable.size, lexems[i].second.c_str(), temp_prefix.c_str(), iddatatype, IT::IDTYPE::F });
						}
						else if (lextable.size >= 1 && lextable.table[lextable.size - 1].lexema == LEX_DATATYPE && param) {
							IT::Add(idtable, { lextable.size, lexems[i].second.c_str(), current_prefix.c_str(), iddatatype, IT::IDTYPE::P });
						}
						else {
...
#pragma region "Запись литералов"
				if ((fst_arr[j].symbol == LEX_LITERAL || fst_arr[j].symbol == LEX_MAIN) && it_id == TI_NULLIDX) {
					if (fst_arr[j].symbol == LEX_LITERAL) {
						...
						if (it_id == TI_NULLIDX)
							IT::Add(idtable, { lextable.size, "", "", iddatatype, IT::IDTYPE::L, lexems[i].second });
					}
					else if (fst_arr[j].symbol == LEX_MAIN)
						IT::Add(idtable, { lextable.size, lexems[i].second.c_str(), "", IT::IDDATATYPE::INT, IT::IDTYPE::F });
				}
#pragma endregion
...
#pragma endregion
				...
				*logfile << "Лексема " << lexems[i].second << " не распознана. Строка " << lexems[i].first << std::endl;
		}
	}
	if (_main == false)
		throw ERROR_THROW(124);
